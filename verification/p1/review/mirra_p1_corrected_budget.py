#!/usr/bin/env python3
"""Reproducible MIRRA P1 N=14 bound derivation; no input files are modified.
...
def compute(rc5_path, scripts_path):
    rc5_bytes = rc5_path.read_bytes()
    scripts_bytes = scripts_path.read_bytes()
    require(hashlib.sha256(rc5_bytes).hexdigest() == RC5_SHA, "unexpected RC5 archive")
    require(hashlib.sha256(scripts_bytes).hexdigest() == SCRIPTS_SHA, "unexpected scripts ZIP")
    with tarfile.open(fileobj=io.BytesIO(rc5_bytes), mode="r:gz") as archive:
        source = archive.extractfile("mirra-rc5/verification/p1/exp_q32_production.py").read()
        wasm = archive.extractfile("mirra-rc5/artifacts/mirra_canister.wasm").read()
    require(hashlib.sha256(wasm).hexdigest() == WASM_SHA, "unexpected RC5 Wasm")
    # This exact hash-pinned source was inspected in the preceding review.
    # __main__ is not executed; no file/network operations occur in this module.
    ...
    with zipfile.ZipFile(io.BytesIO(scripts_bytes)) as archive:
        old_bytes = archive.read("MIRRA_P1_ORIGINAL_SCRIPTS_2026-09-02/logs/p1_proof_artifact.json")
    require(hashlib.sha256(old_bytes).hexdigest() == PROOF_SHA, "old proof drift")
    ...
    return {
        ...
        "inputs": {"rc5_archive_sha256": RC5_SHA, "scripts_zip_sha256": SCRIPTS_SHA,
                   "old_proof_sha256": PROOF_SHA, "unchanged_wasm_sha256": WASM_SHA,
        ...
        "checks": {"model_matches_rc5_and_horner_bound_at_points": len(samples),
                   ...
                   "certificate_or_source_or_pin_modified": False},
    }


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("rc5_archive", type=Path)
    parser.add_argument("scripts_zip", type=Path)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    result = compute(args.rc5_archive, args.scripts_zip)
    if args.json:
